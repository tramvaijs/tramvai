...
export const DocsRating = ({ label }) => {
  ...
  const [shouldLeaveReply, setShouldLeaveReply] = useState(false);
  ...
  const giveFeedback = (value) => {
    ...
    setVoted(true);
    // Предлагаем ввести комментарий если пользователь дизлайкнул
    setShouldLeaveReply(value === 0);
  };

  const leaveReply = (event) => {
    const {
      target: {
        previousSibling: { value },
      },
    } = event;
    if ((window as any).ga && value) {
      (window as any).ga('send', {
        hitType: 'event',
        eventCategory: 'reply',
        eventAction: value,
