...
- [JSON-LD](#jsonld), can be set and dynamically updated both server-side and client-side
...
- if your JSON-LD depends on dynamic data (like page parameters, API responses, etc.), you can update it in runtime both on server and client, just like any other meta-tag. This is especially useful for pages like `/categories/:category`, where structured data must change depending on the current route.
...
### How-to remove default robots meta tags?

In some cases, you may not want any robots meta tags to be configured. In such situations, you must manage them manually. To enable this behavior, use robots:skip in updateMeta or APPLY_META_TOKEN. For example:
...
- `jsonLd` – `<script type="application/ld+json">` tag
- `robots` - function with parameters `(type: 'all' | 'noindex' | 'nofollow' | 'none' | 'skip')` which returns `<meta name="robots" content="none">`, `<meta name="robots" content="noindex, nofollow">` or `<meta name="robots" content="noarchive">` or nothing if you will choose `skip` option
