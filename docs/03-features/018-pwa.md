...
You can define multiple PWA configurations with different scopes:
...
The active PWA configuration is selected at runtime based on the current route. By default, the configuration with the most specific matching scope is used.

Scopes may overlap, but they must not be identical. It is assumed that navigation to a route with a more specific service worker is performed via a full page reload rather than an SPA navigation, since service worker registration, web manifest initialization, and meta tag updates are only performed during the `init` stage.

You can customize the selection algorithm by providing your own implementation for the `PWA_RESOLVE_TOKEN` token:

```ts
import { ExtractDependencyType, provide } from '@tramvai/core';
import {
  PWA_RESOLVE_TOKEN,
...
The default PWA settings are applied to every configuration. Partial configurations are not supported, so it is not possible to configure only a service worker or only a web manifest and meta tags for a specific route.
