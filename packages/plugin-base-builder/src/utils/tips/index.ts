...
import type { BannerOptions, TramvaiTip, TramvaiTipUsageInfo } from './types';
...
export const getTip = (config: ConfigService, options: BannerOptions): TramvaiTip | undefined => {
  ...
  for (const possibleTip of filteredTips) {
    ...
    if (possibleTip.isApplicable(config, options)) {
