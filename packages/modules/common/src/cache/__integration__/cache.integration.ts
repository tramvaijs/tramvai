...
test.describe('cache metrics integration', () => {
  ...
  test('should set max gauge on default cache size', async ({ I, page, app }) => {
    ...
    await test.step('Проверяем что в метриках тестового кэша с дефолтным размером есть max', async () => {
      await test
        .expect(page.locator('body'))
  ...
  test('should increment miss counter for get method', async ({ I, page, app }) => {
    ...
    await test.step('Проверяем что в метриках get метода тестового кэша есть cache miss', async () => {
      await test
        .expect(page.locator('body'))
  ...
  test('should increment hit counter for get method', async ({ I, page, app }) => {
    ...
    await test.step('Проверяем что в метриках get метода тестового кэша есть cache hit', async () => {
      await test
        .expect(page.locator('body'))
  ...
  test('should increment miss counter for has method', async ({ I, page, app }) => {
    ...
    await test.step('Проверяем что в метриках has метода тестового кэша есть cache miss', async () => {
      await test
        .expect(page.locator('body'))
...
test.describe('browser cache', () => {
  test('should provide browser implementations of cache', async ({ I, page, app }) => {
    ...
    await test.step('Проверяем что кэши отработали и на странице выводится значения из кэшей', async () => {
